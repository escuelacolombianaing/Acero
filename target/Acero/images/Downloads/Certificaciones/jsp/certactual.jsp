...
String doc = "", ident = "", senor = "", cadena = "", sol, otro, solic = "", elab = "", valpagpa = "", asig = "", icetex="";
String text = "", sem, valor, matric="", estu= "", pacad = "", sem1, mat = "", beca, ben = "", credvis = "", credtot = "", periodos = "", per_s = "", CambioPl = "";
...
String anterior = "PortalCertific", a = "", res = "", cred = "0", nasig = "0", hor = "0", casig="", regcal = "";
String mess[] = new String[12], dia = "", annio = "", tsem = "", cadena1 = "", pacadb = "", pacad1 = "", cad = "", cadasig = "";
int lon = 0, indp = 0, cant = 0;
long valf = 0, valm = 0;
...
Vector CantAsig = est.CantAsigF(carnet, pacad);
Vector AsigP = est.AsigPend(carnet, pacad);

if (CantAsig.size()>0){
    cant = Integer.parseInt((String) CantAsig.elementAt(0)); //==Cant Asignaturas
    }
    ...
    if(cant == 1){
      for ( int jj = 0 ; jj < AsigP.size() ; jj++ ){
		Vector asigpen = (Vector)AsigP.elementAt(jj);
                cadasig =  (asigpen.elementAt(0).toString()) ;
       }
    } else //=If cant=1
      if(cant == 2){
      for ( int jj = 0 ; jj < AsigP.size() ; jj++ ){
		Vector asigpen = (Vector)AsigP.elementAt(jj);
                if (jj == 0)
                    cadasig =  (asigpen.elementAt(0).toString()) +" y " ;
                else
                    cadasig = cadasig + (asigpen.elementAt(0).toString());
       }
    }else //=If cant=2
      if(cant == 3){
      for ( int jj = 0 ; jj < AsigP.size() ; jj++ ){
		Vector asigpen = (Vector)AsigP.elementAt(jj);
                if (jj == 0)
                    cadasig =  (asigpen.elementAt(0).toString()) + ", " ;
                if (jj == 1)
                    cadasig = cadasig + (asigpen.elementAt(0).toString()) + " y ";
                if (jj == 2)
                    cadasig = cadasig + (asigpen.elementAt(0).toString()) ;
       }
    } //=If cant=3
...
for(int j = 1; j <= div; j++) {
  ...
  cadena = inf.elementAt(1).toString();
   ...
   // String cadena1 = inf.elementAt(14).toString();
    ...
    casig = numero.convertirLetras(cant);
    casig = casig.toLowerCase();

    //String per = inf.elementAt(13).toString();
    //a = per.substring(0, 4);
    a = pacad.substring(0, 4);
    per_s = pers.substring(0, 4);
    //String p = per.substring(5, 6); //el inicial
    String p = pacad.substring(5, 6);
    String ps = pers.substring(5, 6);

    if(ps.equals("1"))
        periodos = "primer período académico";
    else if(ps.equals("2"))
        periodos = "segundo período académico";
    else if(ps.equals("I"))
        periodos = "período intersemestral";
                  ...
                  <td>
                    <p>
                      <center>
                        ...
                        <textarea name="texto3" cols="110" rows="5">Que <%=senor%> ha aprobado <%=credvis%> créditos académicos de un total de <%=credtot%> créditos académicos de su plan de estudios, equivalentes al <%=porcent%> por ciento del programa de <%=inf.elementAt(2)%><%if (icetex.equals("1")){%>. <%}else{%> y cursa según el Reglamento Estudiantil de la Institución el <%=tsem%> semestre académico.<%}%>
                </textarea>
                        <%}
                        }%>
                      </center>
            ...
            <%}%>
              <tr>
                ...
                <%if (beca.equals("1") && becajg.size() > 0){ %>
                 ...
                 <tr>
                  <td>
                      <%if ((cant <= 3) && (beca.equals("0"))){%>
                        <textarea name="texto7" cols="110" rows="3">Que con el estado actual del registro académico para el <%=periodos%> de <%=per_s%>, le queda aún por cursar y aprobar <%=casig%> asignaturas (<%=cadasig%>), para finalizar su plan de estudios.                           
                        </textarea>
                      <%}else{%>
                        <textarea name="texto7" cols="110" rows="3">Que con el estado actual del registro académico para el <%=periodos%> de <%=per_s%>, le queda aún por cursar y aprobar <%=casig%> asignaturas para finalizar su plan de estudios, de acuerdo con el listado anexo.
                        </textarea>
                      <%}%>
                  </td>
                </tr>
